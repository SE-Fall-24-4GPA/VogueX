...
for col in features_to_encode:
    loaded_label_encoders[col] = joblib.load(
        os.path.join(model_save_dir, f"{col}_label_encoder.pkl")
    )
...
encoded_data = {
    col: loaded_label_encoders[col].transform(data[col]) for col in features_to_encode
}
dataset_features = loaded_model(
    {col: tf.constant(encoded_data[col]) for col in features_to_encode}
).numpy()
...
similar_items = find_similar_items(
    input_values, loaded_model, loaded_label_encoders, dataset_features, top_n=5
)
